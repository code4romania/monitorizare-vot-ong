...
export function answerReducer(state = initialAnswerState, action: AnswerActions) {
  switch (action.type) {
    case AnswerActionTypes.LOAD_PREVIEW:
      ...
      const shouldLoadList = shouldLoadPage(action.payload.page, action.payload.pageSize, state.threads.length);
      const shouldUpdateFilters = Object.keys(action.payload.answerFilters).length > 0;
